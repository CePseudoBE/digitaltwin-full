...
local function normalize_roles(user_claims, client_id)
    ...
    local acc, seen = {}, {}
    if user_claims and user_claims.realm_access
        and type(user_claims.realm_access.roles) == "table" then
        for _, r in ipairs(user_claims.realm_access.roles) do
            if not seen[r] then
                seen[r] = true; acc[#acc + 1] = r
            end
        end
    end
    if user_claims and user_claims.resource_access and client_id then
        local ra = user_claims.resource_access[client_id]
...
local function set_identity_headers(ctx, conf, user_claims)
    -- Écraser ce que le client aurait envoyé
    core.request.set_header(ctx, "X-User-Id", "")
    core.request.set_header(ctx, "X-User-Roles", "")

    if not user_claims then return end

    if user_claims.sub then
        core.request.set_header(ctx, "X-User-Id", tostring(user_claims.sub))
    end

    local roles_csv = normalize_roles(user_claims, conf.client_id)
    if roles_csv ~= "" then
        core.request.set_header(ctx, "X-User-Roles", roles_csv)
    end
end
-- === end identity helpers ===

-- Rate limiting function
local function check_rate_limit(key, limit_per_minute)
    if not rate_limit_dict then
        core.log.warn("rate_limit_dict not available")
        return true
    end
    ...
    local current_count, err = rate_limit_dict:get(rate_key)
    if err then
        core.log.error("Failed to get rate limit count: ", err)
        return true
    end
    ...
    local new_count, err = rate_limit_dict:incr(rate_key, 1, 0, 60)
    if err then
        core.log.error("Failed to increment rate limit count: ", err)
        return true
    end
    ...
    properties = {
        ...
        rate_limit = {
            ...
            properties = {
                auth = {
                    type = "integer",
                    description = "Rate limit only for the authenticated users",
                    default = 100
                },
                anon = {
                    type = "integer",
                    description = "Rate limit for anon users",
                    default = 10
                }
            },
            default = { auth = 100, anon = 10 }
        },
    ...
    if not has_token then
        if conf.bearer_only then
            if conf.unauth_action == "pass" then
                core.log.warn("No token found, but bearer_only=true and unauth_action=pass => skipping auth")
...
local function add_access_token_header(ctx, conf, token)
    if not token or not conf.set_access_token_header then return end
    if conf.access_token_in_authorization_header then
        core.request.set_header(ctx, "Authorization", "Bearer " .. token) -- overwrite
    else
        core.request.set_header(ctx, "X-Access-Token", token)             -- overwrite
    end
end
...
function _M.rewrite(plugin_conf, ctx)
    ...
    if conf.bearer_only or conf.introspection_endpoint or conf.public_key or conf.use_jwks then
        ...
        if response then
            ...
            -- ⬇️ Ajoute cette ligne pour pousser X-User-Id / X-User-Roles
            set_identity_headers(ctx, conf, response)
    ...
    if not response then
        ...
        if response then
            ...
            set_identity_headers(ctx, conf, response.user)
